...
export async function exampleSendBlockTransaction() {
    ...
    const mnemonic = 'nice clump primary castle calm arctic oak skill wrong lecture scrap job shine boat any legend achieve push acoustic tragic resource problem river segment'
    const privateKey = PrivateKey.fromMnemonic(mnemonic)
    const pubkey = privateKey.toPubkey()
    const sender = pubkey.toAddress().toAccBech32()
    console.log(privateKey.toHex());
    console.log(pubkey.toHex());
    console.log(`\n privateKey: ${privateKey} | pubkey: ${pubkey} | sender: ${sender} \n`);
